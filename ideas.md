...
### 1.1 Critical
- [x] **Map delta streaming** — terrain destruction is not synced to late-joining clients or after reconnect. Send a compressed terrain diff or re-send full map on reconnect.
...
- [x] **Respawn invincibility** — newly spawned worms can be instantly killed. Add ~60-tick invincibility flash.
- [x] **Spectator mode** — `player.spectating` flag exists but is never acted on; implement spectator-only camera.

### 1.2 Gameplay
- [x] **Aim wrap-around** — when you hold up or down past 90° nothing happens; original Liero wraps to ±90° with a gentle stop. Add a small bounce-back feel.
- [x] **Shell casings stick/bounce** — shells currently use particle gravity but don't bounce off terrain. Give them a `bounce: 60` like a real casing.
- [x] **Bonus drop on kill** — original Liero drops a weapon/health bonus ~17% of kills. The `BONUS.DROP_CHANCE` constant (1700) is defined but unused.
- [x] **Grasshopper special** — the `grasshopper: true` flag exists on the Grasshopper weapon but no special explosive-bounce logic is implemented.
- [x] **Hellraider spawns** — `spawnsProjectiles: true` on Hellraider is defined but the spawn logic is not coded.
- [x] **Blood staining on terrain** — particles stick to terrain but don't permanently stain it. Paint `mapColors` at the particle position when it comes to rest.

### 1.3 Networking
- [x] **Reconnection** — disconnecting and reconnecting loses room context; auto-rejoin the last room.
- [x] **Heartbeat / ping display** — show latency in HUD.
- [x] **Room password** — add an optional password field to Room creation.
...
- [x] **Permanent blood stains** — when a blood particle lands on dirt, paint the `mapColors` cell a red-tinted shade. These persist for the rest of the match.
- [x] **Gib system** — on death, scatter 6–12 large (3×3 pixel) fleshy chunks with high velocity and long lifetime. Use palette indices 80–87 and 172–179 (skin/red tones).
- [x] **Increased base blood** — double the default `bloodOnHit` for all weapons (or add a server-side "goriness" multiplier settable per room).
- [x] **Blood pooling** — accumulate a blood-stain counter per cell; at threshold, change the cell's colour index to a darker red permanently.
- [x] **Wound sprites** — draw a damage overlay (red smear) on the worm sprite when health < 30 (recolor the sprite using a colour-multiply composite operation on Canvas).
- [x] **Explosion gibs** — large explosions (radius ≥ large) scatter body-part sprites (simple 4×4 chunks) in addition to splinter particles.
- [x] **Screen blood splat** — when local player takes heavy damage, flash a translucent red overlay on the HUD canvas.
- [x] **Increased particle cap** — raise `BLOOD.LIMIT` from 500 to 1500 for denser visual effect.
- [x] **Longer blood lifetime** — multiply particle `life` by 1.5× globally for a bloodier, longer-lasting mess.
...
- [x] **Multi-character support** — extend `Worm` with a `character` string field. The client passes it when joining. The renderer loads the matching sprite folder from `public/sprites/<character>/`.
- [x] **Character select screen** — add a character picker after weapon select (grid of thumbnails). Save choice in localStorage.
...
| Weapon | Behaviour |
|--------|-----------|
| **Flamethrower Mk2** | [x] Longer range, leaves burning ground tiles that deal tick damage |
| **Portal Gun** | [x] Two projectiles; entering one portal exits the other (teleport) |
| **Gravity Bomb** | [x] Creates a gravity well that pulls worms and projectiles |
| **Freeze Ray** | [x] Slows enemy movement/aim for 2 seconds |
| **Drill Rocket** | [x] Tunnels through dirt without exploding until it exits |
| **Chain Lightning** | [x] Arcs between nearest worms up to 3 hops |
| **Boomerang** | [x] Returns to the thrower; damages on both trips |
| **Teleport Mine** | [x] On proximity trigger, teleports victim to random map location |
| **Riot Shield** (active) | [x] Blocks incoming projectiles for 1 second while held |
| **Black Hole** | [x] Fires a mini black hole that grows, eats terrain, then collapses |
...
- [x] **Spectator mode** (fix existing stub) — spectators follow any player with camera, no input sent.
...
- [x] **Kill feed improvements** — show weapon icon next to kill notification.
...
- [x] **Private rooms** — password-protected rooms.
- [x] **Bot players** — server-side AI worms that pathfind with A\* on the terrain and pick random targets.
...
- [x] **Minimap** — small map overview (top-right corner) showing worm positions with colour dots.
- [x] **Damage numbers** — floating damage text rising from hit position (toggleable).
- [ ] **Weapon icons** — 16×16 pixel-art icons for each weapon in the HUD slots instead of text.
- [x] **Round-start countdown** — 3-2-1 overlay before players can move.
- [x] **Kill streak announcements** — "DOUBLE KILL", "RAMPAGE" etc. at 2/3/5 consecutive kills.
- [x] **Post-round stats screen** — show accuracy, damage dealt, favourite weapon per player.
...
- [x] **Terrain delta sync** — send only changed terrain cells rather than full map on map changes.
...
1. [x] Server port fix (README / server.js)
2. [x] Map delta sync on reconnect
3. [x] Respawn invincibility
4. [x] Blood staining on terrain
5. [x] Hellraider + Grasshopper weapon logic
6. [x] Bonus drop on kill

### Phase 2 — Bloodier & More Fun
1. [x] Permanent blood stains on terrain
2. [x] Gib system on death
3. [x] Screen blood splat overlay
4. [x] Increased particle cap + lifetime

### Phase 3 — Mods & Content
1. [ ] WebLiero JSON mod loader
2. [x] Character select + sprite packs from itch.io
